...
nucleotide composition: %s%% A, %s%% T, %s%% C and %s%% G.''' %tuple(percent_comp)
...
while True:
	upload_choice = raw_input("Upload primers to Transcriptic? (Y/N)").upper()
	if not re.match("^[Y, N]*$", upload_choice):
		print "Enter \"Y\" or \"N\""
	else:
		break

if upload_choice == "Y":
	csv_file = open('mutagenesis_primers.csv', 'wb')
	fieldnames = ['Label', 'Sequence', 'Scale',
				  'Purification', 'Concentration', 'Diluent']
	writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
	writer.writeheader() 
	f_prim = []
	r_prim = []
	for j in opt_primers:
		f_prim.append({'Label': prim_name[sel_prim_idx[j]],
					 'Sequence': selected_primers[j],
					 'Scale': '10 nM',
					 'Purification': 'HPLC',
					 'Concentration': '100 uM',
					 'Diluent': 'Water'})
		r_prim.append({'Label': prim_name[sel_prim_idx[j]][0:-8] + "_reverse",
					 'Sequence': ("".join(rev_com)),
					 'Scale': '10 nM',
					 'Purification': 'HPLC',
					 'Concentration': '100 uM',
					 'Diluent': 'Water'})
	for i in range(0, len(f_prim)):
		writer.writerow(f_prim[i])
		writer.writerow(r_prim[i])
# Outputs primers in a .CSV format for automates ordering purposes
